...
def _cancel_databricks(databricks_run_id, profile):
    _jobs_runs_cancel(databricks_run_id, profile)


def _monitor_databricks(databricks_run_id, profile, sleep_interval=30):
    """
...
class DatabricksPollableRun(PollableRun):
    ...
    def wait(self):
        return _monitor_databricks(self.databricks_run_id, self.profile)

    def cancel(self):
        _cancel_databricks(self.databricks_run_id, self.profile)
