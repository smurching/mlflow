    ...
    if trace_destination:
        ...
        if isinstance(trace_destination, (UCSchemaLocation, UnityCatalog)):
            ...
            if is_in_databricks_model_serving_environment():
                _logger.warning(
                    "Traces will be exported to the Unity Catalog destination set by "
                    "`MLFLOW_TRACING_DESTINATION`. In Databricks model serving, setting an "
                    "explicit trace destination disables the in-memory trace buffer, so the "
                    "endpoint will return `trace: null` even though traces are logged to UC. "
                    "To return traces in the API response, unset `MLFLOW_TRACING_DESTINATION`."
                )
